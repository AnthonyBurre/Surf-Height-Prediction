...
## Environment
Venv is at `./.venv` (managed by uv). Use `./.venv/bin/python` and `./.venv/bin/pytest` for all Python commands. To install or change deps, run `uv sync --all-extras` (rebuilds `.venv` from `pyproject.toml` + `uv.lock`).
